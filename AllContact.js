...
//     setLoading(true);
//     try {
//       const res = await fetch(`http://localhost:3000/api/mycontacts`, {
//         method: "GET",
//         headers: {
...
const AllContact = () => {
  ...
  const [contacts, setContacts] = useState([]);
  const [searchInput, setSearchInput] = useState("");
  const [originalContacts, setOriginalContacts] = useState([]); // Maintain original contacts for search reset
  ...
  return (
    <>
      <div>
        <h1>Your Contacts</h1>
        <a href="/mycontacts" className="btn btn-danger my-2" onClick={() => window.location.reload()}>
          Reload Contact
        </a>
        <hr className="my-4" />
        {loading ? (
          ...
          <>
            {contacts.length === 0 ? (
              ...
              <>
                ...
                <p>
                  Your Total Contacts: <strong>{originalContacts.length}</strong>
                </p>
                <table className="table table-hover">
                  <thead>
                    <tr className="table-dark">
